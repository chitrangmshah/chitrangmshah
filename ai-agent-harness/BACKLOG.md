...
- [ ] **Consent + private-data access flow.** How a requester authenticates and requests `{private}` values, and how the owner grants / denies. *Leaning:* per-person markdown **consent records** (who / what / purpose / withdrawal; verifiable guardian consent for minors, re-confirmed at 18); the `{private}` marker is the public placeholder this resolves. Ties to ROADMAP "private information / auth".
